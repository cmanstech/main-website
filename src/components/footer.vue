...
<template>
  <footer class="bg-(--secondary-darkbrown) text-(--primary-eggwhite)">
    <!-- TOP: content -->
    <div class="mx-[30px] py-12 grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_auto] gap-10">
      <!-- Left: big copy -->
      <div class="max-w-[min(50vw,720px)]">
        ...
        <p class="mt-10 opacity-90">it@cmans.tech</p>
      </div>

      <!-- Right: links + CTA -->
      <div class="flex flex-col justify-between">
        <ul class="space-y-4 text-[15px] leading-6">
          <li><RouterLink to="/about"    class="footer-link">{{ t('footer.about') }}</RouterLink></li>
          <li><RouterLink to="/projects" class="footer-link">{{ t('footer.projects') }}</RouterLink></li>
        </ul>

        <div class="mt-10 md:mt-16 flex justify-end">
          <RouterLink
            to="/contact"
            class="inline-flex items-center gap-3 rounded-full border border-(--primary-eggwhite)/85 px-5 py-2.5
                   hover:bg-(--primary-eggwhite) hover:text-(--secondary-darkbrown) transition"
          >
            {{ t('footer.contact') }}
