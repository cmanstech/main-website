...
const items = computed(() => tm('servicesLanding.items'))
    ...
    <div class="mx-[30px] max-w-none lg:mx-auto px-0 lg:px-4 py-34 lg:py-44">
      ...
      <div class="grid grid-cols-12 gap-6">
        <div class="col-span-11 lg:col-span-4 flex items-start gap-6">
          <span class="text-[40px] font-(--display-en) text-(--secondary-darkbrown)">{{ t('servicesLanding.label') }}</span>
        </div>
      ...
      <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-11 lg:col-span-7 divide-y divide-(--secondary-darkbrown)/30">
          <div v-for="(it, i) in items" :key="i" class="py-8">
            <button
              type="button"
            ...
            <Transition @enter="enter" @after-enter="afterEnter" @leave="leave">
              <div v-show="active === i" class="overflow-hidden">
                <div class="pt-4 pr-10 text-[15px] leading-6 text-(--secondary-darkbrown)/80">
                  <p class="max-w-[75ch] whitespace-pre-line">{{ it.desc }}</p>
                </div>
              </div>
            </Transition>
          </div>
